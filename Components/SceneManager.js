...
class SceneManager{
  ...
  addLoadingScreen = function(){
    ...
    // Add to div
    loadDiv.appendChild(expOceanImg);
  ...
  update = function(time){
    ...
    if (this.ocean && this.camera.position.y > -4) { // Limiting the updates by camera position does not improve performance in my PC 
      if (this.ocean.isLoaded) {
        ...
        if (this.OceanBuoy !== undefined) {
          if (this.OceanBuoy.isLoaded) {
            ...
            let OceanBuoy = this.OceanBuoy;
            OceanBuoy.root.position.x = OceanBuoy.root.position.x * coef + (1 - coef) * position.x;
            OceanBuoy.root.position.y = OceanBuoy.root.position.y * coef + (1 - coef) * position.y;
            OceanBuoy.root.position.z = OceanBuoy.root.position.z * coef + (1 - coef) * position.z;
            ...
            tempQuat.normalize();
            OceanBuoy.root.quaternion.slerp(tempQuat, 0.002);
          }
        }
      }
    }
