...
export default {
  ...
  methods: {
    ...
    updateCanvas: function(){
      ...
      if (this.timeSpanInHours > this.maxHalfHourlyPoints || this.DataManager.OceanDataRetriever.isLoading){ // Use daily data
        this.isDailyVisible = true;
          ...
          if (ddData != undefined){
            ...
            for (let j = 0; j < measures.length; j++){
              ...
              if (ddData[measures[j]]){
                ...
                let factor = 1;
                let dataType = this.DataManager.OceanDataRetriever.DataTypes[measures[j]];
                if (measures[j].includes('CUR')){
                  dataType = this.DataManager.OceanDataRetriever.DataTypes['CUR'];
                }
                if (dataType){
        ...
        for (let i = 0; i < Math.ceil(numHalfHours); i++) {
          ...
          if (hhData != undefined) {
            ...
            // console.log(Object.keys(this.DataManager.OceanDataRetriever.halfHourlyData));
    ...
    setStartEndDates: function (sDate, eDate) {
      ...
      if (this.timeSpanInHours <= Math.max(this.$refs.dataStreamsCanvas.width, this.maxHalfHourlyPoints)){ 
        ...
        if (!this.DataManager.OceanDataRetriever.isLoading) // Update canvas once all files are loaded
            this.updateCanvas();
        }
    ...
    updateCurrentDate: function(isoString){
      ...
      if (!this.isDailyVisible) { 
        ...
        if (ddData != undefined){
          ddData.timestamp = isoString; // TODO: should remove this if Ocean daily static data is regenerated
